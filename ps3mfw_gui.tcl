...
# Copyright (C) RedDot-3ND7355 (edmayc7@hotmail.fr, edmayc7)
# Copyright (C) HABIB (For sum epic patterns)
# Copyright (C) RazorX (For sum epic tasks)
# Copyright (C) Arachetous (For an fix)
#
# This software is distributed under the terms of the GNU General Public
# License ("GPL") version 3, as published by the Free Software Foundation.
#

# After each update i will change the chkchkdldl value to the new one and if you havent updated the build it will pop a message saying youll have to update!
...
namespace eval ::gui {
    ...
    proc create_gui {arguments selected_tasks} {
		...
		#end of routine
		#End of Full build routine!
		
		#Loading MSG
		#if {[package provide Tk] != "" } {
        #   tk_messageBox -default ok -title "Loading the GUI" -message "Loading... Press OK, But dont touch until it says you can!" -icon warning -detail "This is added for hasters, sorry"
        #} else {
        #   puts "Press \[RETURN\] or \[ENTER\] to continue"
        #   gets stdin
        #}
		
		    bind . <Button-1> {::gui::hello}
		
		proc hello {} {
		tk_messageBox -default ok -title "HEY!" -message "The tool is loading... Do you mind?" -icon warning -detail "Press ok"
		}
		
		
        wm title . "PS3MFW Builder v${::PS3MFW_VERSION} Build: ${PS3MFW_BUILD}"
        create_menu
...
#		exit}
		...
		set update [::ttk::labelframe .left.update -text "[::xml::GetData ${::xmllang} "Lang:Update" 0]"]
		...
		set updatertrol [::ttk::button .right.updatertrol -text ""]
		set pressbutton [::ttk::button .right.updatertrol.label -text "Press to update Builder!" -command ::gui::updater]
        ...
        add_about_msg $about
        pack $about $build $setting $exit $pressbutton $updatertrol -side top -expand false -fill both -padx $::gui::padx -pady $::gui::pady
		...
		#Loading MSG
		#if {[package provide Tk] != "" } {
        #   tk_messageBox -default ok -title "Loaded the GUI" -message "Loaded! You can now begin" -icon info -detail "PS3MFW Builder FTW"
        #} else {
        #   puts "Press \[RETURN\] or \[ENTER\] to continue"
        #   gets stdin
        #}
		
		#Remove button-1 bind
		proc hello {} {
		#Reput left click feature after load!
		}
		::gui::hello
    }
